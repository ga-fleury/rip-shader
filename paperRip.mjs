...
export default class PaperRipAnimation {
  constructor(canvas, {
    color="#292929",
    background= "#292929",
    backgroundOpacity= 0,
    ...
    this.rip = new PaperRip(this.gl, {color,background,backgroundOpacity,texture,amplitude,rippedFrequency,rippedAmplitude,curveFrequency,curveAmplitude,rippedDelta,rippedHeight,horizontal})
  ...
  in() {
    ...
    this.tl = gsap.to(this.rip.uniforms.uProgress, {
      value: 1,
      duration: this.duration,
      ease: this.ease,
      onComplete:()=>{
        document.body.dispatchEvent(new Event('paper-rip'))
      }
    })
  }
  ...
  resizeGL() {
    ...
    this.rip.uniforms.uAspect.value = this.canvasSize.width/this.canvasSize.height;
  }

  onFrame() {
    // render
    this.renderer.render({ scene : this.rip.mesh});

    if(this.isLooping) {
      this.time = (this.time ?? 0) + 0.01

      this.rip.uniforms.uProgress.value = (Math.sin(this.time) + 1) * 0.5
    }
